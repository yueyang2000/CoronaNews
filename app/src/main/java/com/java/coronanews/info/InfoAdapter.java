...
public class InfoAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private List<COVIDInfo> mData = new ArrayList<>();
    ...
    public COVIDInfo getInfo(int position) {
        return mData.get(position);
    }

    public void setData(List<COVIDInfo> data) {
        mData = new ArrayList<COVIDInfo>(data);
        this.notifyDataSetChanged();
    }

    public void appendData(List<COVIDInfo> data) {
        int pos = mData.size();
        mData.addAll(data);
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == TYPE_ITEM) {
            ...
            return new InfoAdapter.ItemViewHolder(view);
        } else {
            View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.footer, parent, false);
            return new InfoAdapter.FooterViewHolder(view);
        }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof ItemViewHolder) {
            COVIDInfo info = mData.get(position);
            final ItemViewHolder item = (ItemViewHolder) holder;
            item.mTitle.setText(info.label);
            item.mAuthor.setText(info.GetDescription());
            item.mDate.setText("");
